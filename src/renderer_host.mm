...
bool writePPM(const fs::path& outputPath, uint32_t width, uint32_t height, const uint8_t* rgba) {
    std::error_code ec;
...
bool renderSceneToFile(const RenderOptions& options, const SceneDescription& scene, std::string* error_message) {
    @autoreleasepool {
        ...
        id<MTLBuffer> outBuffer = [device newBufferWithLength:pixelCount * 4 options:MTLResourceStorageModeShared];
        if (outBuffer == nil) {
        ...
        if (uniformBuffer == nil || cameraBuffer == nil || sphereBuffer == nil || planeBuffer == nil || triangleBuffer == nil) {
        ...
        const auto* pixels = static_cast<const uint8_t*>([outBuffer contents]);
        if (!writePPM(options.output, options.width, options.height, pixels)) {
